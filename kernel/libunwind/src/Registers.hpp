...
public:
  ...
  bool        validFloatRegister(int) const { return false; }
  uint64_t      getFloatRegister(int num) const;
  void        setFloatRegister(int num, uint64_t value);
...
inline uint64_t Registers_x86_64::getFloatRegister(int) const {
  _LIBUNWIND_ABORT("no x86_64 float registers");
}

inline void Registers_x86_64::setFloatRegister(int, uint64_t) {
  _LIBUNWIND_ABORT("no x86_64 float registers");
}
