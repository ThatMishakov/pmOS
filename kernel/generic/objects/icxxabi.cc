...
void *__dso_handle =
    0; // Attention! Optimally, you should remove the '= 0' part and define this in your asm script.
...
void __cxa_finalize(void *f)
{
    ...
    if (!f) {
        ...
        while (i--) {
            if (__atexit_funcs[i].destructor_func) {
                 ...
                 * To make sure we don't call any entries that have already been called and unset at
                 *runtime. Those will contain a value of 0, and calling a function with value 0 will
                 *cause undefined behaviour. Remember that linear address 0, in a non-virtual
    ...
    while (i--) {
         ...
         *algorithm in __cxa_atexit simply inserts the next destructor at the end of the table. So,
         *we have holes with our current algorithm This function should be modified to move all the
         *destructors above the one currently being called and removed one place down in the list,
         *so as to cover up the hole. Otherwise, whenever a destructor is called and removed, an
        ...
        if (__atexit_funcs[i].destructor_func == f) {
             ...
             * In the case of a function that takes no arguments, it will simply be ignore within
             *the function itself. No worries.
             **/
            (*__atexit_funcs[i].destructor_func)(__atexit_funcs[i].obj_ptr);
