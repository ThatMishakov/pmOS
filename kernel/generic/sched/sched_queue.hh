 ...
 * 
 * This structure is a queue of tasks, used for scheduling. It is used to store queues of processes in a different
 * states (ready, blocked, uninited). The process can only be in 1 queue at the time (or in no queue if it is running).
 * The queue is implemented as a doubly-linked list with the pointers inside task descriptor.
 * 
 * The kernel uses one queue for the processes that are uninited. The ready processes are stored in different multilevel queues
 * (currently, there are 16 levels), one global and one per CPU, allowing for processes to be bound to particular CPUs.
 * The blocked processes are stored in local blocked queues.
 * 
 * The functions thet are not prefixed by "atomic_" offer no protection against race conditions. If the structure might be
 * accesed by different CPUs at the same time, the *lock* should be locked before calling them. 
 */
class sched_queue {
public:
    /**
     * Pushes the task descriptor to the front of the queue and sets its parrent queue to *this*.
...
protected:
    ...
    /// Delete move contructor. Since the tasks inside the queue hold pointers to parent queue, it makes it complicated to move it.
    sched_queue(sched_queue&&) = delete;
};

/// Queue for the blocked processes. The same as the normal sched_queue, except that upon deletion it automatically unblocks all the tasks
