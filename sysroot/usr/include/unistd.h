...
enum {
    ...
    F_TLOCK = 2, //< Lock a file region for exclusive use, but return if the region is already locked.
...
enum {
    ...
    _PC_2_SYMLINKS = 7, //< Maximum number of symbolic links that can be traversed in the resolution of a pathname in the absence of a loop.
    _PC_ALLOC_SIZE_MIN = 8, //< Minimum size for a file containing a hole.
    _PC_REC_MAX_XFER_SIZE = 9, //< Recommended increment for file transfer sizes between the processes.
    _PC_REC_MIN_XFER_SIZE = 10, //< Recommended increment for file transfer sizes between the processes.
    _PC_REC_XFER_ALIGN = 11, //< Recommended file transfer buffer alignment.
    _PC_SYMLINK_MAX = 12, //< Maximum length of a symbolic link.
    _PC_CHOWN_RESTRICTED = 13, //< Whether chown requests are restricted to processes with appropriate privileges.
    _PC_NO_TRUNC = 14, //< Whether attempts to create a pathname longer than {NAME_MAX} generate an error.
...
enum {
    ...
    _SC_AIO_PRIO_DELTA_MAX = 3, //< Maximum amount by which a process can decrease its asynchronous I/O priority level.
    ...
    _SC_COLL_WEIGHTS_MAX = 12, //< Maximum number of weights that can be assigned to an entry of the LC_COLLATE order keyword in the locale definition file.
    _SC_DELAYTIMER_MAX = 13, //< Maximum number of timer expiration overruns.
    _SC_EXPR_NEST_MAX = 14, //< Maximum number of expressions that can be nested within parentheses by the expr utility.
    _SC_HOST_NAME_MAX = 15, //< Maximum length of a host name (not including the terminating null) as returned from the gethostname() function.
    _SC_IOV_MAX = 16, //< Maximum number of iovec structures that one process has available for use with readv() or writev().
    ...
    _SC_GETGR_R_SIZE_MAX = 20, //< Initial size of the array used by the getgrnam() and getgrgid() functions.
    _SC_GETPW_R_SIZE_MAX = 21, //< Initial size of the array used by the getpwnam_r() and getpwuid_r() functions.
    ...
    _SC_THREAD_DESTRUCTOR_ITERATIONS = 26, //< Maximum number of attempts made to destroy a thread's thread-specific data values on thread exit.
    ...
    _SC_RE_DUP_MAX = 30, //< Maximum number of repeated occurrences of a regular expression permitted when using the interval notation \{m,n\}.
    ...
    _SC_SIGQUEUE_MAX = 34, //< Maximum number of queued signals that a process may send and have pending at the receiver(s) at any time.
    _SC_STREAM_MAX = 35, //< Maximum number of streams that one process can have open at one time.
    _SC_SYMLOOP_MAX = 36, //< Maximum number of symbolic links that can be traversed in the resolution of a pathname in the absence of a loop.
